import os
from chantstats import load_pieces, ChantStatsConfig
...
pieces_1_thru_8 = load_pieces("plainchant_sequences", cfg, filename_pattern="BN_lat_1112_Sequence_0[1-8]*.xml").pieces
...
piece12 = PlainchantSequencePiece(os.path.join(sequences_dir, prefix+"12_Fulgens_preclara.xml"))        # heavy polymodal frame 1 (no amen formula)
piece14 = PlainchantSequencePiece(os.path.join(sequences_dir, prefix+"14_Zima_uetus.xml"))              # light polymodal frame 1 (with amen formula)
piece32 = PlainchantSequencePiece(os.path.join(sequences_dir, prefix+"32_Magnus_deus.xml"))             # heavy polymodal frame 2 (with amen formula)
piece45 = PlainchantSequencePiece(os.path.join(sequences_dir, prefix+"45_Aue_maria_gratia_plena.xml"))  # light polymodal frame 2 (with amen formula)
