...
def test_grouping_by_modal_category():
    ...
    piece7 = FakePiece(final="E-", ambitus="plagal")
    ...
    assert grouping[("E-", "plagal")].items == [piece7, piece3]


def test_make_results_dataframe():
    notes1 = [Note("A3"), Note("D3"), Note("C3"), Note("D3"), Note("E3")]
    notes2 = [Note("F3"), Note("A3"), Note("G3"), Note("E-3"), Note("F3"), Note("C3"), Note("F3"), Note("B3")]
    ...
    row_piece1 = pd.Series([20.0, 0, 0, 20.0, 40.0, 0, 20.0, 0, 0], index=list(PC))
    row_piece2 = pd.Series([12.5, 0, 12.5, 12.5, 0, 12.5, 0, 37.5, 12.5], index=list(PC))
    row_piece3 = pd.Series([40.0, 0, 0, 20.0, 0, 0, 0, 20.0, 20.0], index=list(PC))
