...
def test_result_descriptor():
    ...
    assert rd.output_dirname == "chant/1_mode_profiles/sequences/pcs"
    assert rd.get_output_dir(output_root_dir) == "/tmp/foo/chant/1_mode_profiles/sequences/pcs"
    assert rd.get_output_dir(output_root_dir) == "/tmp/foo/chant/1_mode_profiles/sequences/pcs"
    assert (
        rd.get_full_output_path(output_root_dir, filename_prefix="dendrogram", filename_suffix="")
        == "/tmp/foo/chant/1_mode_profiles/sequences/pcs/dendrogram__06.G_1.final.png"
    )
    assert (
        rd.get_full_output_path(output_root_dir, filename_prefix="quux", filename_suffix="")
        == "/tmp/foo/chant/1_mode_profiles/sequences/pcs/quux__06.G_1.final.png"
    )
    ...
    assert rd.output_dirname == "chant/2_tendency/sequences/mode_degrees"
    assert (
        rd.get_full_output_path(output_root_dir, filename_prefix="dendrogram", filename_suffix="")
        == "/quux/bar/chant/2_tendency/sequences/mode_degrees/dendrogram__10.C_3.plagal.png"
    )
    assert (
        rd.get_full_output_path(output_root_dir, filename_prefix="foobar", filename_suffix="04")
        == "/quux/bar/chant/2_tendency/sequences/mode_degrees/foobar__10.C_3.plagal__04.png"
    )
