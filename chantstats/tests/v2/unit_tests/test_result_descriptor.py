...
def test_result_descriptor():
    ...
    assert rd.output_dirname == "chant/pc_freqs/sequences/pcs/by_final/final_G"
    assert (
        rd.get_output_dir(output_root_dir, p_cutoff=0.3)
        == "/tmp/foo/p_cutoff_0.30/chant/pc_freqs/sequences/pcs/by_final/final_G"
    )
    assert (
        rd.get_output_dir(output_root_dir, p_cutoff=0.4)
        == "/tmp/foo/p_cutoff_0.40/chant/pc_freqs/sequences/pcs/by_final/final_G"
    )
    assert (
        rd.get_full_output_path(output_root_dir, "dendrogram.png", p_cutoff=0.2)
        == "/tmp/foo/p_cutoff_0.20/chant/pc_freqs/sequences/pcs/by_final/final_G/dendrogram.png"
    )
    assert (
        rd.get_full_output_path(output_root_dir, "quux.png", p_cutoff=0.6)
        == "/tmp/foo/p_cutoff_0.60/chant/pc_freqs/sequences/pcs/by_final/final_G/quux.png"
    )
    ...
    assert rd.output_dirname == "chant/tendency/sequences/mode_degrees/plagal_modes/plagal_C"
    assert (
        rd.get_full_output_path(output_root_dir, "dendrogram.png", p_cutoff=0.15)
        == "/quux/bar/p_cutoff_0.15/chant/tendency/sequences/mode_degrees/plagal_modes/plagal_C/dendrogram.png"
    )
    assert (
        rd.get_full_output_path(output_root_dir, "foobar.png", p_cutoff=0.04)
        == "/quux/bar/p_cutoff_0.04/chant/tendency/sequences/mode_degrees/plagal_modes/plagal_C/foobar.png"
    )
