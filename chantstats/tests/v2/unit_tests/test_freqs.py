...
OCCURRING_PCS = ["A", "B-", "B", "C", "D", "E-", "E", "F", "G"]


def test_pc_freqs():
    notes = [Note("A3"), Note("G3"), Note("A3"), Note("C3"), Note("D3"), Note("C3"), Note("A3"), Note("E-3")]
    pc_freqs = PCFreqs.from_notes(notes)
    abs_freqs_expected = pd.Series([3, 0, 0, 2, 1, 1, 0, 0, 1], index=OCCURRING_PCS)
    rel_freqs_expected = pd.Series([37.5, 0, 0, 25.0, 12.5, 12.5, 0, 0, 12.5], index=OCCURRING_PCS)
...
def test_zero_pc_freqs():
    zero_abs_freqs_expected = pd.Series([0, 0, 0, 0, 0, 0, 0, 0, 0], index=OCCURRING_PCS, dtype=int)
    zero_rel_freqs_expected = pd.Series([NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN], index=OCCURRING_PCS, dtype=float)
...
def test_add_pc_freqs():
    ...
    pc_freqs_sum_expected = PCFreqs(pd.Series([3, 1, 0, 4, 1, 0, 1, 0, 0], index=OCCURRING_PCS))
