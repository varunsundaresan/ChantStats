...
def test_pc_freqs():
    ...
    abs_freqs_expected = pd.Series([1, 0, 0, 1, 3, 1, 0, 2], index=OCCURRING_PCS)
    rel_freqs_expected = pd.Series([12.5, 0, 0, 12.5, 37.5, 12.5, 0, 25.0], index=OCCURRING_PCS)
...
def test_zero_pc_freqs():
    zero_abs_freqs_expected = pd.Series([0, 0, 0, 0, 0, 0, 0, 0], index=OCCURRING_PCS, dtype=int)
    zero_rel_freqs_expected = pd.Series([NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN], index=OCCURRING_PCS, dtype=float)
...
def test_add_pc_freqs():
    ...
    pc_freqs_sum_expected = PCFreqs(pd.Series([1, 1, 0, 0, 3, 1, 0, 4], index=OCCURRING_PCS))
