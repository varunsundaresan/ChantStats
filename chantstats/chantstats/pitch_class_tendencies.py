...
class PCTendencies:
    OCCURRING_PCS = OCCURRING_PCS

    def __init__(self, df_pair_counts):
        assert isinstance(df_pair_counts, pd.DataFrame)
        assert set(df_pair_counts.index) == set(OCCURRING_PCS)
        assert set(df_pair_counts.columns) == set(OCCURRING_PCS)
    ...
    def from_pitch_classes(cls, pcs):
        assert set(pcs).issubset(OCCURRING_PCS)
        s = pd.Series(pcs)
        ...
        df_pair_counts = (
            ...
            .reindex(OCCURRING_PCS)
            .fillna(0, downcast="infer")
            .reindex(OCCURRING_PCS, axis=1)
            .fillna(0, downcast="infer")
        )
